...
const BasicForm = (props) => {
  const {value: firstName, isValid : enteredFirstNameIsValid, isError: isFirstNameError, inputChangeHandler : firstNameInputHandler, inputBlurHandler: firstNameBlurHandler, reset: resetFirstName } = useInput((d) => d.trim().length > 0)

  const {value: lastName, isValid: enteredLastNameIsValid, isError: isLastNameError, inputChangeHandler: lastNameInputHandler, inputBlurHandler: lastNameBlurHandler, reset: resetLastName} = useInput((d) => d.trim().length > 0)
